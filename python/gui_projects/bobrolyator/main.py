...
class Calculator(QMainWindow):
    ...
    def connect_other_buttons(self) -> None:
        ...
        self.ui.btn_koren.clicked.connect(self.koren)
    ...
    def koren(self) -> None:
        ...
        try:
            ...
            result = math.sqrt(float(entry))

            self.avoid_deleting_char_on_negation(str(result))
            self.entry.setText(str(result))
        except ValueError:
            # Handle potential errors like invalid input
            self.show_error(config.ERROR_INVALID_INPUT)
